...
class PrevMatchAdapter(
    ...
    val data: MutableList<MatchDetail>,
    val urlHome: MutableList<BadgeModel>,
    val urlAway: MutableList<BadgeModel>
) :
    RecyclerView.Adapter<PrevMatchAdapter.CustomVH>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PrevMatchAdapter.CustomVH {
    ...
    override fun onBindViewHolder(h: PrevMatchAdapter.CustomVH, i: Int) {
    ...
    private fun removeFromDB(i : Int){
        try {
            context.database.use{
                delete(FavoriteMatch.TABLE_FAV, ("(EVENT_ID = {id})"),"id" to data[i].idEvent.toString())
            }
        }catch (e:Exception){
