...
class NextMatchLoveAdapter(
    val context: Activity,
    val data: List<MatchDetailModel>,
    val urlHome: MutableList<BadgeModel>,
    val urlAway: MutableList<BadgeModel>
) :
    RecyclerView.Adapter<NextMatchLoveAdapter.CustomVH>() {
    ...
    override fun onBindViewHolder(h: NextMatchLoveAdapter.CustomVH, i: Int) {
    ...
    private fun removeFromDB(i: Int) {
        try {
            ...
            callback.removeItemNext(data[i])
        } catch (e: Exception) {
    ...
    interface onRemoveItem {
        fun removeItemNext(model: MatchDetailModel)
    }
}